...
int main(int argc, char *argv[]) {
    ...
    ret = sem_close(write_mutex);
    if (ret)
        handle_error("Error closing write mutex");
    ...
    if (ret)
        handle_error("error removing read mutex");

    ret = sem_unlink(WRITE_MUTEX);
    if (ret)
        handle_error("error removing write mutex");
